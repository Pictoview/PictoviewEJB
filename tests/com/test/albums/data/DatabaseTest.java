...
public class DatabaseTest {
	...
	public void testPhotoAccess() {
		try {
			...
			MediaDTO coverPhoto2 = albumDAO.fetchAlbumCoverPhoto(userIds.get(mockUser1), albumId2);
			MediaDTO coverPhoto3 = albumDAO.fetchAlbumCoverPhoto(userIds.get(mockUser1), albumId3);
			MediaDTO coverPhoto4 = albumDAO.fetchAlbumCoverPhoto(userIds.get(mockUser2), albumId4);
			MediaDTO coverPhoto5 = albumDAO.fetchAlbumCoverPhoto(userIds.get(mockUser2), albumId5);
			MediaDTO coverPhoto6 = albumDAO.fetchAlbumCoverPhoto(userIds.get(mockUser2), albumId6);
			MediaDTO coverPhoto7 = albumDAO.fetchAlbumCoverPhoto(userIds.get(mockUser3), albumId7);
			MediaDTO coverPhoto8 = albumDAO.fetchAlbumCoverPhoto(userIds.get(mockUser3), albumId8);
			MediaDTO coverPhoto9 = albumDAO.fetchAlbumCoverPhoto(userIds.get(mockUser3), albumId9);
	...
	private void checkCorrectPhotoIDs(List<MediaDTO> photos, long... ids) {
		Assert.assertEquals(ids.length, photos.size());
		Set<Long> correctIds = createIdSets(ids);
		for (MediaDTO dto : photos) {
			Assert.assertTrue(correctIds.contains(dto.getId()));
			correctIds.remove(dto.getId());
		}
	}
	
	private void checkCorrectCoverID(MediaDTO cover, long albumId, long coverid) {
